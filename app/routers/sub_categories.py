...
from app.models.sub_categories import SubCategoryRead, SubCategory, SubCategoryCreate, SubCategoryUpdate

router = APIRouter(prefix="/sub_categories", tags=["sub_categories"])


@router.get("/", response_model=list[SubCategoryRead])
async def get_all_sub_categories(*, session: Session = Depends(get_session), category_id : int | None = None):
    if not category_id:
...
async def get_sub_category_by_id(*, session: Session = Depends(get_session), sub_category_id: int):
...
async def create_sub_category(*, session: Session = Depends(get_session), sub_category: SubCategoryCreate):
...
async def update_sub_category(*, session: Session = Depends(get_session), sub_category: SubCategoryUpdate, sub_category_id: int):
...
async def update_sub_category(*, session: Session = Depends(get_session), sub_category_id: int):
    sub_category = session.get(SubCategory, sub_category_id)
    if not sub_category:
        raise HTTPException(status_code=404, detail="Sub Category Not Found")
