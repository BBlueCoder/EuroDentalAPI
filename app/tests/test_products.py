...
def test_create_product(client: TestClient, product_name):
    res = client.post("/products", data={"product_name": product_name})
    assert res.status_code == status.HTTP_200_OK
    assert res.json().get("product_name") == product_name


def test_create_and_get_products(client: TestClient):
    client.post("/products", data={"product_name": "product1"})
    client.post("/products", data={"product_name": "product2"})
...
def test_update_product(client: TestClient, product_name):
    product_data = client.post("/products", data={"product_name": "product"}).json()
...
def test_delete_product(client: TestClient):
    product_data = client.post("/products", data={"product_name": "product1"}).json()
