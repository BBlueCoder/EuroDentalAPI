...
def sub_categories_fixture(categories : list[CategoryRead]):
    ...
    for i, category in enumerate(categories):
        sub_categories.append({"sub_category":f"sub_category{i+1}","category_id":category.id})
...
def test_create_sub_category(client: TestClient, sub_categories):
    res = client.post(SUB_CATEGORIES_PATH,
                      json={"sub_category":sub_categories[0]["sub_category"],
...
def test_get_sub_category_by_id(client : TestClient,sub_categories):
    sub_category_data = client.post(SUB_CATEGORIES_PATH,
                json={"sub_category": sub_categories[0]["sub_category"],
                      "category_id": sub_categories[0]["category_id"]
                      }
                ).json()
...
@pytest.mark.parametrize("sub_category_id", [
    "5"
])
...
@pytest.mark.parametrize("category_id, res_len",[
    (1,1),
    (2, 1),
    (0, 2),
])
def test_create_and_get_sub_categories(client : TestClient, sub_categories, category_id, res_len):
    client.post(SUB_CATEGORIES_PATH,
                json={"sub_category": sub_categories[0]["sub_category"],
                      "category_id": sub_categories[0]["category_id"]
                      }
                )
    client.post(SUB_CATEGORIES_PATH,
                json={"sub_category": sub_categories[1]["sub_category"],
...
def test_update_sub_category(client : TestClient, sub_categories):
    ...
    res = client.put(f"{SUB_CATEGORIES_PATH}/{sub_category_data["id"]}", json={
        "sub_category":"update"
    })
    assert res.status_code == status.HTTP_200_OK
...
def test_update_invalid_sub_category(client: TestClient):
    res = client.put(f"{SUB_CATEGORIES_PATH}/5", json={
        "sub_category": "update"
    })
    assert res.status_code == status.HTTP_404_NOT_FOUND
