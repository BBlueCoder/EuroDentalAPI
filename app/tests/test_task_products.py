...
def task_fixture(session : Session,client_db: Client, user_db: User):
    task_dic = {"task_name": "task1", "client_id": client_db.id, "create_by": user_db.id, "status": "working"}
    task = Task(**task_dic)
    session.add(task)
    session.commit()
...
def test_create_task_products(client: TestClient, product: Product, task : Task):
    task_product = {"product_reference": product.reference, "task_id":task.id, "price":35,"quantity":5}
...
def test_create_and_get_task_products(client: TestClient,product: Product, task : Task):
    for task_product in task_products:
        client.post(TASK_PRODUCTS_PATH, json={"product_reference": product.reference, "task_id":task.id})
...
def test_get_task_product_by_id(client: TestClient,product: Product, task : Task):
    task_product_data = client.post(
        TASK_PRODUCTS_PATH, json={"product_reference": product.reference, "task_id":task.id}
    ).json()
...
def test_update_task_product(client: TestClient, price : float, product: Product, task : Task):
    task_product_data = client.post(TASK_PRODUCTS_PATH,
                                    json={"quantity":5,"product_reference": product.reference, "task_id":task.id}
                                    ).json()
